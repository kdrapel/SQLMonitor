...
namespace Xnlab.SQLMon.Logic
{
    internal class QueryEngine {
        ...
        internal const string SqlJobs =
            "SELECT j.job_id AS spid, j.name AS program_name, CAST(a.last_executed_step_id AS nvarchar(10)) AS dbid, 0 AS cpu, 0 AS physical_io, NULL AS login_time, a.start_execution_date AS last_batch_begin, a.stop_execution_date AS last_batch_end, CASE Enabled WHEN 1 THEN 'Enabled' ELSE 'Disabled' END AS status, @@SERVERNAME AS hostname, @@SPID AS hostprocess, NULL AS cmd, SYSTEM_USER AS loginname, enabled, 0 AS percent_complete FROM msdb.dbo.sysjobs j WITH (NOLOCK) LEFT JOIN msdb.dbo.sysjobactivity a WITH (NOLOCK) on j.job_id = a.job_id WHERE (a.session_id = (SELECT max(session_id) FROM msdb.dbo.sysjobactivity WITH (NOLOCK) WHERE job_id = j.job_id AND start_execution_date IS NOT NULL)) ORDER BY program_name";

        internal const string SqlLockedObjects =
            @"SELECT l.request_session_id AS SPID, s.program_name AS ProgramName, DB_NAME(l.resource_database_id) AS DatabaseName, schema_name(o.schema_id) AS SchemaName, o.name AS ObjectName FROM master.sys.dm_tran_locks l
...
WHERE l.resource_associated_entity_id <> 0 AND o.name IS NOT NULL";
        ...
        private const string SqlSessions =
            @"SELECT e.session_id, DB_NAME(r.database_id) AS database_name, ISNULL(r.status, e.status) AS session_status, e.transaction_isolation_level, ISNULL(r.open_transaction_count, 0) AS open_transaction_count, r.command, ISNULL(r.blocking_session_id, 0) AS blocking_session_id, ISNULL(r.sql_handle, c.most_recent_sql_handle) as sql_handle,
...
ORDER BY MIN(RowNo), LevelRow";
        ...
        internal const string SqlObjectScripts =
            @"Select s.name, s.create_date AS CreateDate, s.modify_date AS ModifyDate, s.type, c.text from syscomments c WITH (NOLOCK) left join sys.objects s WITH (NOLOCK) on c.id = s.object_id LEFT JOIN sysobjects o WITH (NOLOCK) ON c.id=o.id LEFT JOIN sys.schemas u WITH (NOLOCK) ON o.uid = u.schema_id";
...
ORDER BY su.name, so.name";
        ...
        internal static List<string> SystemDatabases { get; } = new List<string> {"master", "msdb", "model", "tempdb"};
        ...
        internal static void GetMemoryInfo(ServerInfo server, out long physicalMemory, out long availableMemory) {
            ...
            if (version > 9) {
                ...
                var row = info.Rows[0];
                physicalMemory = GetSizeLong(info, row,
                    new Dictionary<string, int>
                        {{"total_physical_memory_kb", 1}, {"total_physical_memory_in_bytes", Utils.Size1K}});
                physicalMemory = physicalMemory /= Utils.Size1K;

                availableMemory = GetSizeLong(info, row,
                    new Dictionary<string, int>
                        {{"available_physical_memory_kb", 1}, {"available_physical_memory_in_bytes", Utils.Size1K}});
                ...
                physicalMemory = GetSizeLong(info, row,
                    new Dictionary<string, int>
                        {{"physical_memory_kb", 1}, {"physical_memory_in_bytes", Utils.Size1K}});
                physicalMemory = physicalMemory /= Utils.Size1K;

                availableMemory = GetSizeLong(info, row,
                    new Dictionary<string, int>
                        {{"committed_target_kb", 1}, {"committed_target_in_bytes", Utils.Size1K}});
                availableMemory = availableMemory /= Utils.Size1K;
            }
        ...
        internal static DataTable GetDatabaseInfo(ServerInfo server, string database) {
            if (server.IsAzure) {
                ...
                return SqlHelper.Query(
                    string.Format(
                        "SELECT '{0}' AS DatabaseName, Name AS Logical_Name, Physical_Name, CAST(size AS decimal(30,0))*8 AS Size, state, type FROM sys.database_files",
                        database), server);
            }

            return SqlHelper.Query(
                "SELECT DB_NAME(database_id) AS DatabaseName, Name AS Logical_Name, Physical_Name, CAST(size AS decimal(30,0))*8 AS Size, state, type FROM sys.master_files WITH (NOLOCK) WHERE DB_NAME(database_id) = '" +
                database + "'", server);
        }
        ...
        internal static Dictionary<string, KeyValue<long, long>> GetDiskSpace(ServerInfo server) {
            ...
            databases.AsEnumerable().ForEach(d => {
                if (Convert.ToInt32(d["state"]) == 0) {
                    ...
                    database.AsEnumerable().ForEach(f => {
                            files.Add(new Tuple<bool, string, long>(Convert.ToInt32(f["type"]) == 1,
                                f["physical_name"].ToString(),
                                Convert.ToInt64(Convert.ToDecimal(f["Size"]) / Utils.Size1K)));
                        }
                    );
                }
            });
            ...
            files.ForEach(f => {
                ...
                if (spaces.ContainsKey(drive)) spaces[drive].Value += f.Item3;
            });
            return spaces;
        }
            ...
            databases.AsEnumerable().ForEach(d => {
                if (Convert.ToInt32(d["state"]) == 0) {
                    ...
                    if (!SystemDatabases.Contains(name)) {
                        ...
                        var databaseSpace = new Dictionary<DatabaseFileTypes, long>
                            {{DatabaseFileTypes.Data, 0}, {DatabaseFileTypes.Log, 0}};
                        database.AsEnumerable().ForEach(f => {
                        ...
                        bool? shrink = null;
                        if (databaseSpace[DatabaseFileTypes.Log] > databaseSpace[DatabaseFileTypes.Data] / 100.0 *
                            Settings.Instance.DatabaseDataLogSpaceRatio) {
                            shrink = false;
                        }
                        else {
                        ...
                        if (shrink != null)
                            result.Add(name,
                                new Tuple<long, long, bool>(databaseSpace[DatabaseFileTypes.Log],
                                    databaseSpace[DatabaseFileTypes.Data], (bool) shrink));
                    }
                }
            });
            return result;
        ...
        internal static ServerInfo GetServerInfo(ServerInfo server, string catalog) {
            return new ServerInfo {
                IsAzure = server.IsAzure, AuthType = server.AuthType, Server = server.Server, Database = catalog,
            ...
            databaseList.AsEnumerable().ForEach(db => {
                if (Convert.ToInt32(db["state"]) == 0) {
                    ...
                    if (!SystemDatabases.Contains(name)) {
                        ...
                        var stalls = new[] {dbIsReadStall, dbIsWriteStall, logIsReadStall, logIsWriteStall};
                        var max = stalls.Max();
                        var isExceeded = max >= DbStallThreshold;
                        result.Add(new DatabaseStall {
                            Database = name, DbReadStall = dbIsReadStall, DbWriteStall = dbIsWriteStall,
                            LogReadStall = logIsReadStall, LogwriteStall = logIsWriteStall, Max = max,
        ...
        internal static void GetCpuInfo(ServerInfo server, out int sqlProcess, out int systemIdle,
            out int otherProcesses) {
...
select @s as FullStatement, SUBSTRING(@s, (@start/2)+1, ((CASE @end WHEN -1 THEN DATALENGTH(@s) ELSE @end END - @start)/2)+1) as CurrentStatement",
        ...
        internal static DataTable GetDatabaseIoInfo(ServerInfo server) {
            ...
            if (server.IsAzure)
                ...
                sql = string.Format(
                    @"SELECT sys.databases.database_id AS dbid, sys.master_files.file_id AS fileid, sys.master_files.physical_name AS filename
...
WHERE sys.databases.name = '{0}'", server.Database);
            ...
            dataFiles.AsEnumerable().ForEach(d => {
                ...
                var fileStats = SqlHelper.Query(
                    string.Format(
                        "SELECT DATEADD(ss, -1 * Timestamp/1000 , getdate()) AS StartDate, IoStallReadMS, IoStallWriteMS, NumberReads, BytesRead, NumberWrites, BytesWritten FROM ::fn_virtualfilestats({0}, {1})",
                        dbId, fileId), server);
                fileStats.AsEnumerable().ForEach(f => {
                    data.Rows[index]["StartDate"] = f["StartDate"];
                    data.Rows[index]["NumberReads"] =
                        Convert.ToInt64(data.Rows[index]["NumberReads"]) + Convert.ToInt64(f["NumberReads"]);
                    if (Convert.ToInt64(data.Rows[index]["NumberReads"]) == 0)
                        data.Rows[index]["NumberReads"] = 1;
                    data.Rows[index]["BytesRead"] =
                        Convert.ToInt64(data.Rows[index]["BytesRead"]) + Convert.ToInt64(f["BytesRead"]);
                    data.Rows[index]["NumberWrites"] = Convert.ToInt64(data.Rows[index]["NumberWrites"]) +
                                                       Convert.ToInt64(f["NumberWrites"]);
                    if (Convert.ToDouble(data.Rows[index]["NumberWrites"]) == 0)
                        data.Rows[index]["NumberWrites"] = 1;
                    data.Rows[index]["BytesWritten"] = Convert.ToInt64(data.Rows[index]["BytesWritten"]) +
                                                       Convert.ToInt64(f["BytesWritten"]);
                    data.Rows[index]["FileCount"] = Convert.ToInt64(data.Rows[index]["FileCount"]) + 1;
                    data.Rows[index]["IoStallReadMS"] = Convert.ToInt64(data.Rows[index]["IoStallReadMS"]) +
                                                        Convert.ToInt64(f["IoStallReadMS"]);
                    data.Rows[index]["IoStallWriteMS"] = Convert.ToInt64(data.Rows[index]["IoStallWriteMS"]) +
                                                         Convert.ToInt64(f["IoStallWriteMS"]);

                    data.Rows[index]["IsReadStall"] = Convert.ToDouble(data.Rows[index]["IoStallReadMS"]) /
                                                      Convert.ToInt64(data.Rows[index]["NumberReads"]);
                    data.Rows[index]["IsWriteStall"] = Convert.ToDouble(data.Rows[index]["IoStallWriteMS"]) /
                                                       Convert.ToInt64(data.Rows[index]["NumberWrites"]);
                    data.Rows[index]["IsStall"] =
                        (Convert.ToDouble(data.Rows[index]["IoStallReadMS"]) +
                         Convert.ToDouble(data.Rows[index]["IoStallWriteMS"])) /
                        (Convert.ToInt64(data.Rows[index]["NumberReads"]) +
                         Convert.ToInt64(data.Rows[index]["NumberWrites"]));
                });
            });
