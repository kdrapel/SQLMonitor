...
namespace Xnlab.SQLMon.UI
{
    public partial class ConnectionDialog : BaseDialog {
        ...
        public AuthTypes AuthType {
            get => (AuthTypes) cboAuthTypes.SelectedItem;
        ...
        private ServerInfo GetServerInfo => new ServerInfo
            {AuthType = AuthType, Server = Server, User = UserName, Password = Password, Database = "master"};
        ...
        private bool IsSqlServer2005OrAbove() {
            try {
                ...
                if (!is2005OrAbove)
                    Monitor.Instance.ShowMessage(
                        string.Format("Current version {0}, only SQL Server 2005 or above is supported.", version));
