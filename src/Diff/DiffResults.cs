...
namespace Xnlab.SQLMon.Diff
{
    ...
    public class DiffResults : Form {
        ...
        public DiffResults() {
            ...
            Icon = Icon.FromHandle(Resources.History2.GetHicon());
        ...
        private void TextDiff(int previousVersion, int currentVersion) {
            ...
            var destinationScript =
                currentVersion != Utils.EmptyIndex ? GetVersionScript(currentVersion) : string.Empty;
        ...
        private void ShowDiff(DiffListText source, DiffListText destination, List<DiffResultSpan> diffLines,
                ...
                switch (drs.Status) {
                    case DiffResultSpanStatus.DeleteSource:
                        ...
                        for (i = 0; i < drs.Length; i++) {
                            sourceLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) source.GetByIndex(drs.SourceIndex + i)).Line, Color.Red));
                            destinationLines.Add(new KeyValuePair<string, Color>(
                                new string(' ', ((TextLine) source.GetByIndex(drs.SourceIndex + i)).Line.Length),
                                Color.Blue));
                        }
                    ...
                    case DiffResultSpanStatus.NoChange:
                        for (i = 0; i < drs.Length; i++) {
                            sourceLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) source.GetByIndex(drs.SourceIndex + i)).Line, Color.Empty));
                            destinationLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) destination.GetByIndex(drs.DestIndex + i)).Line, Color.Empty));
                        }
                    ...
                    case DiffResultSpanStatus.AddDestination:
                        ...
                        for (i = 0; i < drs.Length; i++) {
                            sourceLines.Add(new KeyValuePair<string, Color>(
                                new string(' ', ((TextLine) destination.GetByIndex(drs.DestIndex + i)).Line.Length),
                                Color.Blue));
                            destinationLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) destination.GetByIndex(drs.DestIndex + i)).Line, Color.Blue));
                        }
                    ...
                    case DiffResultSpanStatus.Replace:
                        ...
                        for (i = 0; i < drs.Length; i++) {
                            sourceLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) source.GetByIndex(drs.SourceIndex + i)).Line, Color.Red));
                            destinationLines.Add(new KeyValuePair<string, Color>(
                                ((TextLine) destination.GetByIndex(drs.DestIndex + i)).Line, Color.Green));
                        }
            ...
            Text = _differences.Count + " differences.";
        }

        private void FillScript(List<KeyValuePair<string, Color>> lines, TextEditorControl textBox) {
            var script = new StringBuilder();
            foreach (var item in lines) script.AppendLine(item.Key);
            textBox.Text = script.ToString();
            for (var i = 0; i < lines.Count; i++)
                if (lines[i].Value == Color.Blue
                    || lines[i].Value == Color.Green
                    || lines[i].Value == Color.Red) {
                    var marker = new TextMarker(textBox.Document.LineSegmentCollection[i].Offset,
                        textBox.Document.LineSegmentCollection[i].Length, TextMarkerType.SolidBlock, Color.White,
                        lines[i].Value);
        ...
        private void InitializeComponent() {
            ...
            //
            this._rtbDestinationScript.Anchor =
                ((System.Windows.Forms.AnchorStyles) ((((System.Windows.Forms.AnchorStyles.Top |
                                                         System.Windows.Forms.AnchorStyles.Bottom)
                                                        | System.Windows.Forms.AnchorStyles.Left)
                                                       | System.Windows.Forms.AnchorStyles.Right)));
            ...
            this._cboDestinations.SelectedIndexChanged +=
                new System.EventHandler(this.OnDestinationsSelectedIndexChanged);
            //
            // lblDestination
            //
        ...
        private void ShowVersionDiff() {
            if (_isLoaded)
                TextDiff(_previousVersion != Utils.EmptyIndex && _cboSources.SelectedValue != null
                        ? Convert.ToInt32(_cboSources.SelectedValue)
                        : Utils.EmptyIndex
        ...
        private void SetDifferencePosition(bool isNext) {
            ...
            if (_currentIndex < _differences.Count) {
                _rtbSourceScript.ActiveTextAreaControl.TextArea.Caret.Position =
                    new TextLocation(0, _differences[_currentIndex]);
