    ...
    public class DiffEngine {
        ...
        private int GetSourceMatchLength(int destIndex, int sourceIndex, int maxLength) {
            ...
            for (matchCount = 0; matchCount < maxLength; matchCount++)
                if (_dest.GetByIndex(destIndex + matchCount).CompareTo(_source.GetByIndex(sourceIndex + matchCount)) !=
                    0)
                    break;
            return matchCount;
        }

        private void GetLongestSourceMatch(DiffState curItem, int destIndex, int destEnd, int sourceStart,
            int sourceEnd) {
            ...
            for (var sourceIndex = sourceStart; sourceIndex <= sourceEnd; sourceIndex++) {
                ...
                if (maxLength <= curBestLength) break;
        ...
        public List<DiffResultSpan> DiffReport() {
            ...
            if (dcount == 0) {
                if (scount > 0) retval.Add(DiffResultSpan.CreateDeleteSource(0, scount));
                return retval;
            }
